...
const contracts = {
  SUPERVISOR: {
    ...
    dataScopes: {
      ...
      PLATFORM: scope('STORE_LIST', ['TEST-STORE-01', 'TEST-STORE-02']),
    },
    expectedMenus: ['督导工作台', '仓库中心', '门店详情', '督导巡店', '整改复核', '培训考试', '平台配置'],
    forbiddenMenus: ['运营工作台', '运营中心', '财务工作台', '老板工作台', '账号权限', '每日报损'],
    routeDenials: ['/daily-loss'],
    routeAllows: [
  ...
  STORE_MANAGER: {
    ...
    forbiddenMenus: ['运营工作台', '平台配置', '老板工作台', '财务工作台', '账号权限', '整改复核', '每日报损'],
    routeDenials: ['/operations', '/platform-login', { from: '/boss', to: '/store' }, '/finance', '/daily-loss'],
    routeAllows: [
  ...
  WAREHOUSE: {
    ...
    forbiddenMenus: ['运营工作台', '平台配置', '培训考试', '财务工作台', '老板工作台', '账号权限', '每日报损'],
    routeDenials: ['/operations', '/platform-login', '/boss', '/finance', '/store', '/daily-loss'],
  },
  FINANCE: {
    name: '财务验收',
    label: '财务',
    ...
    expectedMenus: ['财务工作台', '利润概览', '利润表', '数据录入', '报销栏', '员工工资', '数据导出', '每日报损'],
    forbiddenMenus: ['运营工作台', '平台配置', '培训考试', '老板工作台', '账号权限', '门店工作台'],
    routeDenials: ['/operations', '/platform-login', '/boss', '/store'],
    routeAllows: [{ from: '/daily-loss', to: '/daily-loss' }],
  },
  EMPLOYEE: {
    name: '员工验收',
    label: '员工',
    defaultWorkspace: '/employee',
    // 模拟错误的个人授权；员工仍不得取得报损查询或导出入口。
    permissions: ['exam.learn', 'daily_loss.read', 'daily_loss.export'],
    dataScopes: {
      STORE: scope('OWN_STORE', ['TEST-STORE-01']),
      EXAM: scope('OWN_STORE', ['TEST-STORE-01']),
    },
    expectedMenus: ['员工工作台', '培训考试', '我的资料/工资'],
    forbiddenMenus: ['每日报损', '运营工作台', '财务工作台', '老板工作台', '账号权限'],
    routeDenials: ['/daily-loss', '/operations', '/platform-login', '/boss', '/finance', '/store'],
  },
  BOSS: {
    name: '老板验收',
    label: '老板（系统管理员）',
    ...
    expectedMenus: ['老板工作台', '利润概览', '仓库中心', '每日报损', '门店详情', '督导巡店', '培训考试', '平台配置', '账号权限'],
    forbiddenMenus: [],
    routeAllows: [
