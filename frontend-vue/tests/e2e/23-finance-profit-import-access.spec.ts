...
test('店长仅保留本店手工录入，遗留导入权限也不能打开财务导入直链', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  const requests = await prepare(page, managerSession)

  await page.goto('/data-entry')
  await expect(page.getByRole('heading', { name: '本店经营录入', level: 1 })).toBeVisible()
  await expect(page.getByLabel('当前录入门店')).toContainText('荆州之星店')
  await expect(page.locator('#profit-sales')).toBeVisible()
  ...
  await expect(page.getByRole('alert')).toHaveText('经营数据导入仅限财务或老板处理。店长可继续手工录入本店经营数据。')
...
test('财务从工作台和菜单进入导入月度汇总，自动打开同一校验抽屉', async ({ page }) => {
  ...
  await expect(page.locator('.app-sidebar--desktop')).toContainText('导入月度汇总')

  await importWorkspaceLink.click()
  await expect.poll(() => new URL(page.url()).pathname).toBe('/finance/import')
  await expect(page.getByRole('heading', { name: '导入月度汇总', level: 1 })).toBeVisible()
  await expect(page.getByRole('region', { name: '导入范围' })).toBeVisible()
...
test('老板保留导入月度汇总入口和直链访问', async ({ page }) => {
