      ...
      >
        ...
        <div class="preview-modal__body">
          ...
          <template v-else>
            ...
            <div v-else-if="candidates.length" class="empty-state compact">所有可选员工都符合当前工资生成条件。</div>
            <div v-else class="empty-state compact">当前没有可生成的员工，请补齐考勤或检查员工范围。</div>
          </template>
        </div>
        <ModalFooter>
          <template #info>
            <span v-if="selectedCount">本次将生成已选择的 {{ selectedCount }} 名员工，已有工资记录不会重复生成。</span>
...
@media (max-width: 520px) {
  ...
  .candidate-section > header { display: grid; }
