...
if (-not $ArchiveRoot) {
  $ArchiveRoot = Join-Path $projectRoot "output\vue3-release-check"
}

$archiveDir = Join-Path $ArchiveRoot $timestamp
$logFile = Join-Path $archiveDir "release-check.log"
...
function Archive-PlaywrightReport {
  Write-Log "Archiving Playwright report..."
...
function Write-Summary {
  ...
  $lines = @(
    "AI Profit OS Vue3 release check",
    "Status: $status",
    ...
    "- npm run test:e2e",
    "",
    "Note: write E2E is disabled by default and this script does not write MySQL data."
  )
  Set-Content -LiteralPath $summaryFile -Value $lines -Encoding UTF8
}

Write-Log "AI Profit OS Vue3 release verification"
...
try {
  ...
  Invoke-NpmStep "Vue3 production build" @("run", "build")
  ...
  Invoke-NpmStep "Vue3 full E2E" @("run", "test:e2e")
  $passed = $true
} catch {
...
if ($passed) {
  Write-Host ""
  Write-Host "Vue3 release check passed. Report directory: $archiveDir"
  exit 0
}

Write-Host ""
Write-Host "Vue3 release check failed. Failed step: $failedStep"
