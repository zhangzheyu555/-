...
set @sql := if(
  exists(select 1 from information_schema.columns where table_schema = database() and table_name = 'loss_item_config' and column_name = 'warehouse_category_id'),
  'select 1',
  'alter table loss_item_config add column warehouse_category_id bigint null after category'
);
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql := if(
  exists(select 1 from information_schema.statistics where table_schema = database() and table_name = 'loss_item_config' and index_name = 'idx_loss_item_config_warehouse_category'),
  'select 1',
  'alter table loss_item_config add index idx_loss_item_config_warehouse_category (tenant_id, warehouse_category_id, active)'
);
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

set @sql := if(
  exists(select 1 from information_schema.table_constraints where table_schema = database() and table_name = 'loss_item_config' and constraint_name = 'fk_loss_item_config_warehouse_category'),
  'select 1',
  'alter table loss_item_config add constraint fk_loss_item_config_warehouse_category foreign key (warehouse_category_id) references warehouse_item_category(id)'
);
prepare stmt from @sql; execute stmt; deallocate prepare stmt;

insert into warehouse_item_category(tenant_id, name, parent_id, sort_order, enabled, created_at)
select tenants.tenant_id, required_category.name, null, required_category.sort_order, 1, current_timestamp
from (
...
join warehouse_item_category category
...
set config.warehouse_category_id = category.id,
