...
alter table training_exam_attempt
  add column campaign_id bigint null after paper_id,
  add column assignment_id bigint null after campaign_id;

create index idx_exam_attempt_campaign on training_exam_attempt(tenant_id, campaign_id, submitted_at);
create index idx_exam_attempt_assignment on training_exam_attempt(tenant_id, assignment_id);

alter table training_exam_attempt
  add constraint fk_exam_attempt_campaign foreign key (campaign_id) references training_exam_campaign(id),
  add constraint fk_exam_attempt_assignment foreign key (assignment_id) references training_exam_assignment(id);

alter table training_exam_assignment
  add constraint fk_exam_assignment_attempt foreign key (attempt_id) references training_exam_attempt(id);
