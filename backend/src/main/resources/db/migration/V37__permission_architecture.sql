alter table auth_user
  add column permission_version bigint not null default 1 after enabled;

alter table auth_token
  add column permission_version bigint not null default 1 after user_id;
...
create table if not exists permission_catalog (
  ...
  index idx_permission_catalog_module (module_code, sort_order),
  index idx_permission_catalog_enabled (enabled, module_code),
...
create table if not exists user_permission_override (
  ...
  created_at timestamp not null default current_timestamp,
  updated_at timestamp null default null on update current_timestamp,
  unique key uk_user_permission_override (tenant_id, user_id, permission_code),
  index idx_user_permission_override_effect (tenant_id, user_id, effect),
  index idx_user_permission_override_permission (permission_code),
...
create table if not exists user_data_scope (
  ...
  created_at timestamp not null default current_timestamp,
  updated_at timestamp null default null on update current_timestamp,
  unique key uk_user_data_scope (tenant_id, user_id, domain_code),
  index idx_user_data_scope_domain (tenant_id, domain_code, scope_type),
  ...
  constraint chk_user_data_scope_json check (
    (scope_type = 'STORE_LIST' and scope_value_json is not null
      and json_valid(scope_value_json) and json_type(scope_value_json) = 'ARRAY')
...
from auth_user
join (
  select 'inspection.read' permission_code
...
drop temporary table if exists v37_effective_store_scope;

create temporary table v37_effective_store_scope as
