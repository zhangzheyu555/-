-- Remove only untouched V4/V7 warehouse demo records. Any manual edit or
-- business reference keeps the row. V28 training content has no reliable
-- immutable seed marker, so it is intentionally preserved.
...
where seed_item.tenant_id = 1
  ...
  and seed_batch.expiry_date = date_add(
    seed_batch.received_date, interval seed_item.shelf_life_days day
  )
...
where supplier.tenant_id = 1
  ...
  and not exists (
    ...
    where attachment.tenant_id = supplier.tenant_id
      and attachment.business_id = cast(supplier.id as char)
  )
  and not exists (
    select 1 from warehouse_request_dedup request_dedup
    where request_dedup.tenant_id = supplier.tenant_id
      and request_dedup.business_id = cast(supplier.id as char)
  )
  and not exists (
    select 1 from operation_log log_entry
    where log_entry.tenant_id = supplier.tenant_id
      and log_entry.target_id = cast(supplier.id as char)
  );
...
drop temporary table v59_legacy_default_supplier_candidate;
drop temporary table v59_legacy_demo_item_candidate;
