-- H2 verification equivalent of MySQL V82. The H2 migration chain omits MySQL-only releases.

insert into operation_log(
  tenant_id, operator_id, operator_name, action, target_type, target_id, store_id, month,
  before_json, after_json, reason, created_at
)
select account.tenant_id, null, '系统迁移', '修复督导门店范围', 'auth_user', cast(account.id as varchar(120)), null, null,
       '{"scope":"NONE_OR_PARTIAL"}',
       '{"scope":"ALL_EXISTING_STORES","domains":["STORE","WAREHOUSE","INSPECTION","EXAM","PLATFORM"]}',
       'V78 修复督导角色合并后未同步全部门店数据范围的问题', current_timestamp
...
delete from user_data_scope
where domain_code in ('STORE', 'WAREHOUSE', 'INSPECTION', 'EXAM', 'PLATFORM')
  and exists (
    select 1
    from auth_user account
