-- H2 test equivalent of V41.

drop table if exists v41_inspection_scoring_precheck;
create table v41_inspection_scoring_precheck (
  valid_flag int not null check (valid_flag = 1)
);

insert into v41_inspection_scoring_precheck(valid_flag)
...
from (
  ...
  having count(*) = (
     ...
     and (
       abs(case when max(version.full_score) = 100.00
         then round(sum(snapshot.suggested_score) * 2, 2)
         else sum(snapshot.suggested_score)
       end - 200.00) > 0.01
       or abs(case when max(version.full_score) = 100.00
         then round(sum(snapshot.actual_score) * 2, 2)
         else sum(snapshot.actual_score)
       end - round(max(record.score) * 2, 2)) > 0.01
     )
) inconsistent_complete_snapshot;
...
drop table v41_inspection_scoring_precheck;

alter table inspection_record
  add column pass_score decimal(8,2) not null default 180.00;
...
update inspection_record set pass_score = 180.00 where pass_score <> 180.00;
