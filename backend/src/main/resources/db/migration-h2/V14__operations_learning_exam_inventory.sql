...
alter table training_exam_question add constraint if not exists uk_exam_question_order unique (tenant_id, paper_id, sort_order);
...
-- H2 不支持 MySQL 用户变量，试卷 id 改为在插入时用标量子查询获取。
...
values
  (1, (select id from training_exam_paper where tenant_id = 1 and paper_code = 'NEW_STAFF_BASIC' limit 1), 'SINGLE_CHOICE', '水果出现明显异味时应该怎么处理？', '["继续使用但减少用量","立即停用并记录","混入新鲜水果中使用","只用于外卖订单"]', '立即停用并记录', null, 25, 1, 1, current_timestamp),
  (1, (select id from training_exam_paper where tenant_id = 1 and paper_code = 'NEW_STAFF_BASIC' limit 1), 'SINGLE_CHOICE', '半成品开封后最重要的管理动作是什么？', '["贴标签记录时间","放在吧台方便取用","混合不同批次","只看外观不需要记录"]', '贴标签记录时间', null, 25, 2, 1, current_timestamp),
  (1, (select id from training_exam_paper where tenant_id = 1 and paper_code = 'NEW_STAFF_BASIC' limit 1), 'SINGLE_CHOICE', '店铺盘存金额的基础计算方式是什么？', '["数量 × 单价","销量 × 折扣","收入 - 房租","库存 ÷ 杯量"]', '数量 × 单价', null, 25, 3, 1, current_timestamp),
  (1, (select id from training_exam_paper where tenant_id = 1 and paper_code = 'NEW_STAFF_BASIC' limit 1), 'SINGLE_CHOICE', '饿了么订单金额如果标记为示例数据，运营应该如何理解？', '["可以作为真实财务入账","只做页面演示，不能当真实经营数据","自动写入利润表","代表所有门店已接入"]', '只做页面演示，不能当真实经营数据', null, 25, 4, 1, current_timestamp)
on duplicate key update
